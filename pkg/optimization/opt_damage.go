...
func (stats *SubstatOptimizerDetails) evaluateDPS() float64 {
	...
	_, err := optstats.RunWithConfigCustomStats(
		context.TODO(), stats.cfg, stats.file, stats.simcfg, stats.gcsl,
		stats.simopt, seed, optstats.OptimizerDmgStat, a.Add,
	)
	if err != nil {
		stats.optimizer.logger.Fatal(err.Error())
	}
	a.Flush()
	return mean(a.ExpectedDps)
}
...
func (stats *SubstatOptimizerDetails) optimizeNonERSubstats() []string {
	...
	stats.simcfg.Settings.Iterations = 10
	stats.simcfg.Characters = stats.charProfilesCopy
...
func (stats *SubstatOptimizerDetails) optimizeNonErSubstatsForChar(
	...
	for totalSubs < target {
		...
		stats.charProfilesCopy[idxChar].Stats[best] +=
			stats.substatValues[best] * stats.charSubstatRarityMod[idxChar]
		totalSubs++
